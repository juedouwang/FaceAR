...
- `src/EffectFactory.js`
  ...
  - 男性/女性数字替身主路径为 Kenney CC0 GLB 头部模型。
...
- `src/EffectManager.js`
  ...
  - 暴露 `getRuntimeEffectDebug()`，验证脚本可确认当前加载的是 `kenney-glb-head-only`，并通过 `avatarLoadState = kenney-glb-loaded` 验证 GLB 实际加载完成。
...
- 运行资产：从转换后的模型中提取头部区域，去掉脖子以下和肩部区域，保存为 `assets/avatars/kenney-protagonists/kenney-protagonist-head-only.glb`。
...
- 活动 track 数：4。
- 身份绑定 track 数：2。
- FPS：身份绑定验证约 60；性能 profile 平均约 65。
- 数字替身渲染器：`kenney-glb-head-only`。
