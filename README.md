...
- Choose one privacy action per identity:
  ...
  - `Privacy blur shield`
- Render male/female Kenney CC0 GLB head-bust digital substitutes with face-size fitting, with a procedural Three.js fallback if the model cannot load.
...
was converted to GLB and reduced to a head/neck/upper-shoulder bust so it can load through the existing Three.js r112
`GLTFLoader`. The renderer keeps the existing `avatarMale` and `avatarFemale` action IDs, but internally uses a
`kenney-glb-head-bust` path:

- load the extracted Kenney head-bust GLB and apply the male/female skin texture;
...
camera/local video
  ...
  -> Kenney GLB head-bust / procedural fallback / privacy shield rendering
```
...
- 男性/女性数字替身使用 Kenney CC0 开源角色资源提取出的 GLB 头部/肩颈半身像进行实时覆盖渲染；如果模型加载失败，会退回到程序化 Three.js 3D 数字头。
